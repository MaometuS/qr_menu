...
func TestInteractor_CreateMenu(t *testing.T) {
	...
	estRep.On("GetOne", context.Background(), int64(1)).Return(establishment, nil)
	rep := &repository.MenuRepositoryMock{}
	rep.On("CreateMenu", context.Background(), menu).Return(errors.New("error1"))
	...
	estRep.On("GetOne", context.Background(), int64(1)).Return(establishment, nil)
	rep = &repository.MenuRepositoryMock{}
	rep.On("CreateMenu", context.Background(), menu).Return(nil)
	...
	for i, el := range cases {
		...
		err := interactor.CreateMenu(context.Background(), el.profileID, el.menu)
