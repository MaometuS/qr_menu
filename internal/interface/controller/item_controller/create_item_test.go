...
type createItemTestCase struct {
	req        *http.Request
...
func buildCreateItemTestCase(item models.Item, code int, hasInteractor bool, err error) (*createItemTestCase, error) {
	body := &bytes.Buffer{}
	...
	req := httptest.NewRequest("POST", "/create_item/", body)
	...
	if hasInteractor {
		...
		item.Image = ""
		interactor.On("CreateItem", context.WithValue(req.Context(), "db", nil), int64(1), &item).Return(err)
	}

	return &createItemTestCase{
		req:        req,
		resp:       resp,
		code:       code,
...
func TestItemController_CreateItem(t *testing.T) {
	cases := make([]createItemTestCase, 0)

	cases = append(cases, createItemTestCase{
	...
	testCase, err := buildCreateItemTestCase(models.Item{
		Name:           "name",
		Weight:         "weight",
		Price:          100,
	...
	for i, el := range cases {
		...
		controller.CreateItem(el.resp, el.req)
