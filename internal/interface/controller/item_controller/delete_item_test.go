...
func TestItemController_DeleteItem(t *testing.T) {
	...
	cases = append(cases, testCase{
		req:        httptest.NewRequest("POST", "/delete_item/", nil),
	...
	req = httptest.NewRequest("POST", "/delete_item?id=2&category_id=2", nil)
	req = req.WithContext(context.WithValue(req.Context(), "id", int64(1)))
	resp = httptest.NewRecorder()
	interactor := item_interactor.NewTestImplementation()
	interactor.On("DeleteItem", context.WithValue(req.Context(), "db", nil), int64(1), int64(2)).Return(nil)
	...
	resp = httptest.NewRecorder()
	interactor = item_interactor.NewTestImplementation()
	interactor.On("DeleteItem", context.WithValue(req.Context(), "db", nil), int64(1), int64(2)).Return(errors.New(""))
	...
	for i, el := range cases {
		...
		controller.DeleteItem(el.resp, el.req)
