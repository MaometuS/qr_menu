...
func TestMenuController_DeleteMenu(t *testing.T) {
	...
	req = httptest.NewRequest("POST", "/delete_menu?id=2&establishment_id=2", nil)
	req = req.WithContext(context.WithValue(req.Context(), "id", int64(1)))
	resp = httptest.NewRecorder()
	interactor := menu_interactor.NewTestImplementation()
	interactor.On("DeleteMenu", context.WithValue(req.Context(), "db", nil), int64(1), int64(2)).Return(nil)
	...
	req = httptest.NewRequest("POST", "/delete_item?id=2&establishment_id=2", nil)
	req = req.WithContext(context.WithValue(req.Context(), "id", int64(1)))
	resp = httptest.NewRecorder()
	interactor = menu_interactor.NewTestImplementation()
	interactor.On("DeleteMenu", context.WithValue(req.Context(), "db", nil), int64(1), int64(2)).Return(errors.New(""))
	...
	for i, el := range cases {
		...
		controller.DeleteMenu(el.resp, el.req)
