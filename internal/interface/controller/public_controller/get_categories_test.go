...
func TestPublicController_GetCategories(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/get_categories/", nil)
	...
	req = httptest.NewRequest("GET", "/get_categories?menu_id=2", nil)
	resp = httptest.NewRecorder()
	interactor := public_interactor.NewTestImplementation()
	interactor.On("GetCategories", context.WithValue(req.Context(), "db", nil), resp, int64(2)).Return(nil)
	...
	resp = httptest.NewRecorder()
	interactor = public_interactor.NewTestImplementation()
	interactor.On("GetCategories", context.WithValue(req.Context(), "db", nil), resp, int64(2)).Return(errors.New(""))
