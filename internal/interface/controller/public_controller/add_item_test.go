...
func TestPublicController_AddItem(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/add_item/", nil)
	...
	req = httptest.NewRequest("GET", "/add_item?id=2", nil)
	resp = httptest.NewRecorder()
	...
	for i, el := range cases {
		...
		controller.AddItem(el.resp, el.req)
