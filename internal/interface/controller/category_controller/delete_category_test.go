...
func TestCategoryController_DeleteCategory(t *testing.T) {
	...
	interactor.On("DeleteCategory", context.WithValue(req.Context(), "db", nil), int64(1), int64(2)).Return(nil)
	...
	resp = httptest.NewRecorder()
	interactor = category_interactor.NewTestImplementation()
	interactor.On("DeleteCategory", context.WithValue(req.Context(), "db", nil), int64(1), int64(2)).Return(errors.New(""))
	...
	for i, el := range cases {
		...
		controller.DeleteCategory(el.resp, el.req)
