...
func TestController_EstablishmentInfo(t *testing.T) {
	...
	cases = append(cases, testCase{
		req:        httptest.NewRequest("GET", "/establishment_info/", nil),
	...
	req = httptest.NewRequest("GET", "/establishment_info?id=2", nil)
	req = req.WithContext(context.WithValue(req.Context(), "id", int64(1)))
	resp = httptest.NewRecorder()
	interactor := establishment_interactor.NewTestImplementation()
	interactor.On("EstablishmentInfo", context.WithValue(req.Context(), "db", nil), resp, int64(1), int64(2)).Return(nil)
	...
	resp = httptest.NewRecorder()
	interactor = establishment_interactor.NewTestImplementation()
	interactor.On("EstablishmentInfo", context.WithValue(req.Context(), "db", nil), resp, int64(1), int64(2)).Return(errors.New(""))
	...
	for i, el := range cases {
		...
		controller.EstablishmentInfo(el.resp, el.req)
