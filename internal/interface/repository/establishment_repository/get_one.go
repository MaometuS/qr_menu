...
func (e *establishmentRepository) GetOne(ctx context.Context, id int64) (*models.Establishment, error) {
	db, ok := ctx.Value("db").(entity.PgxIface)
	if !ok {
		return nil, errors.New("db not found in context")
	}

	rows, err := db.Query(ctx, "select * from establishments where id = $1", id)
	if err != nil {
		return nil, err
	}
