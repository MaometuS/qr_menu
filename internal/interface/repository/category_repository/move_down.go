...
func (c *categoryRepository) MoveDown(ctx context.Context, categoryID int64) error {
	...
	t, err := db.Exec(ctx, "update categories set order_param=order_param-1 where order_param = $1 and menu_id = $2", order+1, menuID)
	if err != nil {
		return err
	}
