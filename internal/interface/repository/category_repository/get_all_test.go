...
func TestCategoryRepository_GetAll(t *testing.T) {
	...
	mock.ExpectQuery(
		"select * from categories where menu_id = $1 order by order_param",
	).WithArgs(int64(1)).WillReturnRows(
		pgxmock.NewRows(
			[]string{"id", "name", "background", "menu_id", "is_visible", "order_param"},
		).AddRow(
			int64(1), "name", "background", int64(1), true, int64(1),
		).AddRow(
			int64(2), "name", "background", int64(1), true, int64(2),
		),
	)
	...
	for i, el := range cases {
		...
		if !reflect.DeepEqual(res, el.output.categories) {
